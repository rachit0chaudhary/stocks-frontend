  ...
  useEffect(() => {
    const fetchClientData = async () => {
      ...
      const config = {
        ...
        url: `http://65.0.127.149:5000/api/var/masterAdmin/getClient/${id}`,
        headers: {
    ...
    const fetchTradeData = async (clientId) => {
      ...
      const config = {
        method: "get",
        maxBodyLength: Infinity,
        url: `http://65.0.127.149:5000/api/var/client/trades/client/brokerage/${id}`,
        headers: {
          Authorization: `Bearer ${token}`,
        },
      };
        ...
        mcxTrades.map(async (trade) => {
          try {
            const lotConfig = {
              method: "get",
              maxBodyLength: Infinity,
              url: `http://65.0.127.149:5000/api/var/client/stocks/${trade.instrumentIdentifier}`,
              headers: {
                Authorization: `Bearer ${token}`,
              },
            };
