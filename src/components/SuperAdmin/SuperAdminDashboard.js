...
const SuperAdminDashboard = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const responseSuperAdmin = await fetch(
          "http://65.0.127.149:5000/api/var/superAdmin/getSuperAdmin",
          { headers: myHeaders }
        );
        ...
        const responseClients = await fetch(
          "http://65.0.127.149:5000/api/var/superAdmin/getAllClients",
          { headers: myHeaders }
        );
  ...
  const handleDelete = async (adminId) => {
    ...
    try {
      const response = await fetch(
        `http://65.0.127.149:5000/api/var/superAdmin/delete-masterAdmin/${adminId}`,
        requestOptions
      );
      if (!response.ok) {
  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    try {
      const response = await fetch(
        `http://65.0.127.149:5000/api/var/superAdmin/update-masterAdmin/${selectedAdmin._id}`,
        requestOptions
      );
