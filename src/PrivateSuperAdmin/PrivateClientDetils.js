...
const getToken = () => localStorage.getItem("****xxx*****");
  ...
  useEffect(() => {
    const fetchClientData = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/api/var/privateAdmin/getClientById/${id}`,
          {
            headers: {
    ...
    const fetchTradesData = async () => {
      try {
        const token = getToken();
        const response = await axios.get(
          `${API_BASE_URL}/api/var/privateAdmin/trades/${id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    const fetchRealTimeData = async (instrumentIdentifier) => {
      try {
        const token = getToken();
        const response = await axios.get(
          `${API_BASE_URL}/api/var/privateAdmin/stocks/${instrumentIdentifier}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    const fetchTradeHistory = async () => {
      try {
        const token = getToken();
        const response = await axios.get(
          `${API_BASE_URL}/api/var/privateAdmin/trades/brokerage/${id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const handleDeleteTrade = async (tradeId) => {
    ...
    if (confirmed) {
      try {
        const token = getToken();
        await axios.delete(
          `${API_BASE_URL}/api/var/privateAdmin/trades/${tradeId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        <div className="mt-16 p-4">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full bg-white table-auto border-collapse mb-6">
              <thead>
                <tr className="bg-gradient-to-b from-gray-800 to-black text-white">
                  <th className="p-2">#</th>
...
export default PrivateClientDetils;
